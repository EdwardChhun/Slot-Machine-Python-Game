...
def get_bets():
    while True:
        bets = input("How much would you like bet on each line: $")
        if bets.isdigit():
            bets = int(bets)
            if MIN_BET <= bets <= MAX_BET:
                break
            else:
                print("Amount must be in between the bet spread.")
        else: 
            print("Please enter you bets.")
    return bets
...
def main():
    ...
    while True:
        ...
        if answer == "q":
            ...
            print("Please Press Enter To Play / Q to Quit: ")
        else:
